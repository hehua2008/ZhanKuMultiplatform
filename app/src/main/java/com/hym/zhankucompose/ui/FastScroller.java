...
public class FastScroller extends RecyclerView.ItemDecoration implements RecyclerView.OnItemTouchListener {
    ...
    private int mVerticalRangeOffset = 0;
    private int mHorizontalRangeOffset = 0;
    ...
    public FastScroller(RecyclerView recyclerView, StateListDrawable verticalThumbDrawable,
        ...
        mDefaultWidth = res.getDimensionPixelSize(R.dimen.fastscroll_default_thickness);
        mVerticalThumbWidth = Math.max(mDefaultWidth, verticalThumbDrawable.getIntrinsicWidth());
        mVerticalTrackWidth = Math.max(mDefaultWidth, verticalTrackDrawable.getIntrinsicWidth());
        mHorizontalThumbHeight = Math
            .max(mDefaultWidth, horizontalThumbDrawable.getIntrinsicWidth());
        mHorizontalTrackHeight = Math
            .max(mDefaultWidth, horizontalTrackDrawable.getIntrinsicWidth());
        mScrollbarMinimumRange = res.getDimensionPixelSize(R.dimen.fastscroll_minimum_range);
        mMargin = res.getDimensionPixelOffset(R.dimen.fastscroll_margin);
    ...
    void updateScrollPosition(int offsetX, int offsetY) {
        ...
        if (mNeedVerticalScrollbar) {
            ...
            int calculateVerticalThumbHeight = Math.min(verticalVisibleLength,
                    (verticalVisibleLength * verticalVisibleLength) / verticalContentLength);
            mVerticalThumbHeight = Math.max(mDefaultWidth * 4, calculateVerticalThumbHeight);
            mVerticalRangeOffset = mVerticalThumbHeight - calculateVerticalThumbHeight;
            int verticalThumbRange = verticalVisibleLength - mVerticalRangeOffset;
            mVerticalThumbCenterY = mVerticalRangeOffset / 2 +
                (int) ((verticalThumbRange * middleScreenPos) / verticalContentLength);
        }

        if (mNeedHorizontalScrollbar) {
            float middleScreenPos = offsetX + horizontalVisibleLength / 2.0f;
            int calculateHorizontalThumbWidth = Math.min(horizontalVisibleLength,
                    (horizontalVisibleLength * horizontalVisibleLength) / horizontalContentLength);
            mHorizontalThumbWidth = Math.max(mDefaultWidth * 4, calculateHorizontalThumbWidth);
            mHorizontalRangeOffset = mHorizontalThumbWidth - calculateHorizontalThumbWidth;
            int horizontalThumbRange = horizontalVisibleLength - mHorizontalRangeOffset;
            mHorizontalThumbCenterX = mHorizontalRangeOffset / 2 +
                (int) ((horizontalThumbRange * middleScreenPos) / horizontalContentLength);
        }
    ...
    private int[] getVerticalRange() {
        mVerticalRange[0] = mMargin + mVerticalRangeOffset / 2;
        mVerticalRange[1] = mRecyclerViewHeight - mMargin - mVerticalRangeOffset / 2;
    ...
    private int[] getHorizontalRange() {
        mHorizontalRange[0] = mMargin + mHorizontalRangeOffset / 2;
        mHorizontalRange[1] = mRecyclerViewWidth - mMargin - mHorizontalRangeOffset / 2;
